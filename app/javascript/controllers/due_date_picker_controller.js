...
export default class extends Controller {
  ...
  connect() {
    this.selectedDate = this.dateValue
      ? new Date(this.dateValue + "T00:00:00")
      : null;
  ...
  open(event) {
    ...
    this.pendingSelection = this.selectedDate
      ? new Date(this.selectedDate)
      : null;
    this.viewingDate = this.selectedDate
      ? new Date(this.selectedDate)
  ...
  renderDay(date, isOtherMonth) {
    ...
    const isSelected =
      ...
      compareDate.getTime() ===
        new Date(this.pendingSelection).setHours(0, 0, 0, 0);
  ...
  updateSaveButton() {
    ...
    if (saveBtn) {
      ...
      const isDifferent =
        !this.selectedDate ||
        this.formatISO(this.pendingSelection) !==
